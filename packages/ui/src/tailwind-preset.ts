...
/**
 * The instrument-panel theme, shared by every app in this repo.
 *
 * Colours resolve through CSS custom properties defined in styles.css, so a
 * token moves in one file and both apps move with it. Each app supplies its
 * own `content` globs — that is the only part of a Tailwind config that is
 * genuinely per-app, since it depends on where that app keeps its source.
 */
export const instrumentPanel: Omit<Config, 'content'> = {
  darkMode: ['class'],
  theme: {
    container: {
      center: true,
