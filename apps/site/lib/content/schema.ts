...
/**
 * Visibility, permission and date shape come from @mohdaslam/content-rules,
 * which the emcee app reads from as well. Re-exported here so this module
 * stays the one import site for anything describing portfolio content.
 */
export { permissionStatusSchema, visibilitySchema }
 ...
 */
/**
 * Whether an item will actually reach a public page — the predicate that
 * decides which authoring rules apply below.
 *
 * This used to restate the rule and carry a comment asking the next reader
 * to keep it in step with the runtime gate. It now *is* the runtime gate:
 * isPublished() is the same function the query layer and the emcee app call,
 * so the build-time rules and what actually publishes cannot disagree.
 */
