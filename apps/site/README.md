...
```bash
npm ci                       # from the repository root — this is a workspace
cp .env.example .env.local   # optional — the site runs without it
npm run dev --workspace @mohdaslam/site
```
...
Server components by default. Four things ship client JavaScript: the mobile
navigation drawer, the theme toggle, the contact form, and the share button
on `/work` and each case study. The share *dialog* — Radix plus the QR
encoder — is code-split and fetched on the first press, so a route that
merely shows a share button pays about 9 kB, and the routes that show none
pay nothing.

The design system itself lives in [`packages/ui`](../../packages/ui) and the
publication rules in [`packages/content-rules`](../../packages/content-rules),
both shared with the emcee app. See the [root README](../../README.md).
...
lib/share/
  share-links.ts       server-only: /p/<code> ↔ slug
  qr.ts                QR encoding, loaded with the share dialog
```
...
Every published project has a short link — `mohdaslam.dev/p/5s59ew` — reached
from a Share button on each card in `/work` and in the header of each case
study. The dialog shows the link, a copy button, a QR code and a way into the
page itself; on a phone it is a bottom sheet, and it hands off to the system
share sheet where one exists.

The code is **derived, not stored**: `sha256(namespace + slug)` rendered in a
Crockford-style base32 and cut to six characters. There is no database here,
so a stored token would mean inventing persistence for a site that has none.
Deriving it means the same slug produces the same code on every machine and
every deploy, forever, with nothing to migrate.

Two things follow that are worth stating plainly:

- **A share code is not a key.** `/p/<code>` is a 308 to `/work/<slug>`,
  which is already public. Only published projects get codes, so a link can
  never reach past `getPublicProjects()` — an unpublished project has no
  code, no route and no entry in the map. Nothing about sharing weakens the
  publication boundary, because sharing does not touch it.
- **Codes survive new work.** Slugs are walked in sorted order and a
  colliding slug lengthens its own code rather than everyone shifting, so
  adding a project never invalidates a link already printed on something.

`/p/<code>` is deliberately absent from the sitemap: it redirects to a URL
already listed there, and advertising both would offer a search engine two
URLs for one page.
...
`/privacy` · `/p/[code]`, plus `not-found` and `error`.

`/emcee` and `/emcee/[slug]` are permanent redirects to the emcee site — see
[`apps/emcee`](../emcee). Those paths were live on this site before the run
sheets moved, so they still resolve rather than 404ing.
