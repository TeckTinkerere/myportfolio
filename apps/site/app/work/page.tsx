...
import { ProjectShare } from '@/components/share/project-share'
    ...
    <Section>
      ...
      {visible.length > 0 ? (
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {/* The archive is where someone goes to send a link to a specific
              piece of work, so this is the one listing that carries the
              share control — see the note on ProjectCard's `action` prop. */}
          {visible.map((project) => (
            <ProjectCard
              key={project.slug}
              project={project}
